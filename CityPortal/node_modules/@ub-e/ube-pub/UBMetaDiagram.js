    ...
    me.editPnl = Ext.create('Ext.panel.Panel', {
      ...
      listeners: {
        boxready: function (/* sender */) {
          UB.inject('models/UBE/mxGraph/mxClient' + (window.isDeveloperMode ? '' : '.min') + '.js')
          .then(function () {
            me.initMXEditor()
          })
        },
  ...
  showEntityForm: function (entityCode) {
    ...
    if (entity) {
      ...
      Ext.Object.each(entity.mixins, function (code, row) {
        var dat = Ext.clone(row)
        dat.code = code
        dat.icon = UBE.UBMetaDiagram.mixinPictures[code] || code
        mixinData.push(dat)
      })
  ...
  generateObjHtml: function (cell) {
    ...
    _.forEach(ubObj.mixins, function (mixinConfig, mixinName) {
      if (mixinConfig.enabled) {
        cellMixinImages.push(UBE.UBMetaDiagram.mixinPictures[mixinName] || mixinName)
      }
    })
...
window.mxBasePath = 'models/UBE/mxGraph/src'
UB.ux.UBDocument.editors.ubDiagram = 'UBE.UBMetaDiagram'
UB.ux.UBDocument.contentTypeMapping['application/ubMetaDiagram'] = 'UBE.UBMetaDiagram'
UB.ux.UBDocument.contentTypeMapping['application/ubmetadiagram'] = 'UBE.UBMetaDiagram'
