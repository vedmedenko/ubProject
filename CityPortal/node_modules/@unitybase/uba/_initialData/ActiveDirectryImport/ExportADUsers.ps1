...
$csvfile = $path + "\ALLADUsers.csv"  # "\ALLADUsers_$logDate.csv"
...
$SearchBase = ""
$arrN = $domainName.split(".")
for ($i=0; $i -lt $arrN.length; $i++){ 
  if ($i -ne 0){
     $SearchBase = $SearchBase + ","
  }
  $SearchBase = $SearchBase + "DC=" + $arrN[$i]
}
...
$AllADUsers = Get-ADUser -searchbase $SearchBase -Filter {GivenName -like '*' -and Surname -like '*' -and Enabled -eq 'TRUE'}  # * -Properties * | Where-Object {$_.info -NE 'Migrated'} #ensures that updated users are never exported.
